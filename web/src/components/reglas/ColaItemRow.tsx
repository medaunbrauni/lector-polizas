...
export default function ColaItemRow({
  ...
  return (
    <div className="border border-[var(--color-border)] rounded-2xl overflow-hidden bg-[var(--color-bg-primary)] shadow-sm">

      {/* ── Fila principal ─────────────────────────────────────── */}
      <div className="flex items-start gap-3 px-4 py-3">
        {/* Nombre archivo */}
        <div className="flex-1 min-w-0">
          <p className="text-sm font-medium text-[var(--color-text-primary)] truncate">{item.nombre_archivo}</p>
          <p className="text-xs text-[var(--color-text-secondary)] mt-0.5">
            ...
            {item.razon_ia && <span className="ml-1.5 text-[var(--color-text-secondary)]" title={item.razon_ia}>· {item.razon_ia.slice(0, 60)}…</span>}
          </p>
        </div>
        ...
        <div className="text-xs text-[var(--color-text-secondary)] text-right flex-shrink-0 min-w-[160px]">
          {item.estado === 'enviado' || item.estado === 'confirmado' ? (
            <div className="text-[var(--color-success-text)]">
            ...
            <div className="text-[var(--color-warning-text)] font-medium">
        ...
        <div className="flex items-center gap-1 flex-shrink-0">
          {!terminado && (
            <>
              ...
              <button
                ...
                className={`p-1.5 rounded-lg text-xs transition-colors ${overrideOpen ? 'bg-[var(--color-nav-active-bg)] text-[var(--color-nav-active-text)]' : 'hover:bg-[var(--color-bg-secondary)] text-[var(--color-text-secondary)]'}`}
              >
              ...
              {(item.estado === 'clasificado' || (overrideOpen && ov?.subramoId)) && (
                <button
                  onClick={onConfirmar}
                  className="flex items-center gap-1 px-2.5 py-1.5 text-xs font-medium text-white bg-[var(--color-brand-blue)] hover:opacity-90 rounded-lg transition-opacity"
                >
            ...
            <>
              {linkVer(item) && (
                <Link
                  ...
                  className="flex items-center gap-1 px-2.5 py-1.5 text-xs font-medium text-[var(--color-text-secondary)] hover:bg-[var(--color-bg-secondary)] rounded-lg transition-colors"
                >
              ...
              <button
                ...
                className="flex items-center gap-1 px-2.5 py-1.5 text-xs font-medium text-[var(--color-text-secondary)] hover:bg-[var(--color-bg-secondary)] rounded-lg transition-colors disabled:opacity-50"
              >
          ...
          <button
            onClick={onDescartar}
            title="Eliminar"
            className="p-1.5 hover:bg-red-500/10 text-[var(--color-text-secondary)] hover:text-red-500 rounded-lg transition-colors"
          >
        ...
        <button
          ...
          className={`flex items-center gap-1 px-2 py-1 text-xs rounded-lg border transition-colors flex-shrink-0
            ${previewOpen ? 'border-[var(--color-brand-blue)] text-[var(--color-nav-active-text)] bg-[var(--color-nav-active-bg)]' : 'border-[var(--color-border)] text-[var(--color-text-secondary)] hover:bg-[var(--color-bg-secondary)]'}`}
        >
      ...
      {/* ── Preview de PDF ──────────────────────────────────────── */}
      {previewOpen && (
        <div className="mx-4 mb-3 relative h-[480px] border border-[var(--color-border)] rounded-xl overflow-hidden">
      ...
      {/* ── Override inline ────────────────────────────────────── */}
      {overrideOpen && ov && (
        <div className="mx-4 mb-3 px-4 py-3 bg-[var(--color-bg-secondary)] border border-[var(--color-border)] rounded-xl">
          <p className="text-xs font-semibold text-[var(--color-text-secondary)] uppercase mb-2">Corregir clasificación</p>
          <div className="flex gap-2 flex-wrap">
            {/* Compañía */}
            <select
              value={ov.companiaId}
              onChange={(e) => onCompaniaChange(e.target.value)}
              className="text-sm border border-[var(--color-border)] rounded-lg px-2 py-1.5 bg-[var(--color-bg-primary)] text-[var(--color-text-primary)]"
            >
            ...
            <select
              ...
              className="text-sm border border-[var(--color-border)] rounded-lg px-2 py-1.5 bg-[var(--color-bg-primary)] text-[var(--color-text-primary)] disabled:opacity-40"
            >
            ...
            {ov.subramoId && (
              <button
                onClick={onConfirmar}
                className="flex items-center gap-1 px-3 py-1.5 text-xs font-medium text-white bg-[var(--color-brand-blue)] hover:opacity-90 rounded-lg transition-opacity"
              >
                <CheckCircle2 className="w-3.5 h-3.5" />
